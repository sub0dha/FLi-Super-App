...
2. **Token Generation** → On successful login, backend generates a **JWT token**.
3. **Secure API Calls** → Token is included in headers (`Authorization: Bearer <token>`) for protected routes.
4. **Role-based Access** – Admin has elevated privileges (manage products, orders, discounts).
